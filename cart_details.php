<?php
	...
	if(isset($_SESSION['user'])){
		...
		try{
			$total = 0;
			$stmt = $conn->prepare("SELECT *, cart.id AS cartid, DATEDIFF(to_date, from_date) AS date_difference, products.name AS product_name FROM cart LEFT JOIN products ON products.id=cart.product_id LEFT JOIN category ON category.id=products.category_id WHERE user_id=:user");
			$stmt->execute(['user'=>$user['id']]);
			foreach($stmt as $row){
				$image = (!empty($row['photo'])) ? 'images/'.$row['photo'] : 'images/noimage.jpg';
				$subtotal = $row['price']*$row['quantity']*$row['date_difference'];
				$total += $subtotal;
